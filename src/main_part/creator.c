...
/*   Updated: 2022/11/20 17:29:57 by lter-zak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philosophers.h"

int	go_to_sleep(t_philo *philo, t_philo_gen *philo_gen)
{
	unsigned long long int eat_time;
	long long int time_now = 0;
	eat_time = (unsigned long long)philo_gen->time_eat;
	pthread_mutex_lock(&(philo_gen->sleep));
	printf("time -> %lld ,%d has sleeping\n", ft_time() - philo_gen->start_time, philo->id);
	ft_usleep(philo_gen->time_sleep);
	while(!(philo_gen->check_die))
	{
		if (eat_time  <= ft_time() - time_now)
				break ;	
	}
	pthread_mutex_unlock(&(philo_gen->sleep));
	return(0);
}
